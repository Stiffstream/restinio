...
inline lextokens_t lexer (string_view_t str) {
	...
	for (string_view_t::const_iterator c = str.begin(); c != str.end();) {
		std::size_t _index = c - str.data();
		lextoken_type_t _type;
		if (*c == '*' || *c == '+' || *c == '?') {
		...
		if (*c == ':') {
			...
			while (j < str.size()) {
				unsigned code = static_cast<int>(str.at(j));
...
inline optional_t<const std::string> try_consume (lextoken_type_t type, lextokens_t::const_iterator& token) {
	if (token->type == type) {
		return (token++)->value;
	}
...
inline optional_t<const std::string> consume_text (lextokens_t::const_iterator& token) {
	std::string result{};
...
parse( string_view_t route_sv, const options_t & options )
{
	...
	for (lextokens_t::const_iterator token = tokens.begin(); token != tokens.end();) {
		...
		if (name || pattern) {
			...
			optional_t<const std::string> modifier = try_consume(lextoken_type_t::modifier, token);
			if (name) {
